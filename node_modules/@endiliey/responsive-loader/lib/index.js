...
module.exports = function loader(content) {
  ...
  const placeholderSize = parseInt(config.placeholderSize, 10) || 40; // JPEG compression

  const quality = parseInt(config.quality, 10) || 85; // Useful when converting from PNG to JPG

  const background = config.background; // Specify mimetype to convert to another format
  ...
  const loaderContext = this; // The config that is passed to the adatpers
  ...
  if (config.disable) {
    ...
    const {
      outputPath,
      publicPath
    } = getOutputAndPublicPath(fileName, config);
